...
class ComicTileMenuOption {
  final String title;
  final IconData icon;
  final void Function(BuildContext context, String? comicId) onTap;
...
abstract class ComicTile extends StatelessWidget {
  ...
  void showBlockPane(BuildContext context) {
    // showDialog(
  ...
  void onLongTap_(BuildContext context) {
    // bool favorite = false;
    // showDialog(
    //   context: App.globalContext!,
    //   builder: (context) {
  ...
  void onSecondaryTap_(BuildContext context,TapDownDetails details) {
  ...
  Widget _buildDetailedMode(BuildContext context) {
    return LayoutBuilder(builder: (context, constrains) {
      ...
      return InkWell(
          ...
          onLongPress: enableLongPressed ? () => onLongTap_(context) : null,
          onSecondaryTapDown: (details) => onSecondaryTap_(context, details),
          child: Padding(
...
class NormalComicTile extends ComicTile {
  ...
  @override
  void onLongTap_(BuildContext context) => onLongTap?.call(context);
...
//TODO: 以下代码需要重构  
  ...
  @override
  void onTap_(BuildContext context) => onTap(context);
...
String? isBlocked(BaseComic item) {
  for (var word in appdata.blockingKeyword) {
    ...
    for (var tag in item.tags) {
      ...
      if (item.enableTagsTranslation && tag == word) {
        return word;
      }
    }
  }
  ...
  Widget build(BuildContext context) {
    var content = Column(
      ...
      children: [
        ...
        TextButton(onPressed: onSubmit, child: Text("提交")),
        const SizedBox(height: 16),
      ],
    );
